...
import { ReturnService, ReturnRequest, ReturnStatus, AppealStatus, MediaAttachment } from "@/lib/services/returnService";
      ...
      <div className="container mx-auto px-4 py-16">
        <div className="max-w-4xl mx-auto">
          <div className="text-center">
            ...
            <h2 className="text-2xl font-semibold mb-2">No Return Request Found</h2>
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="space-y-6">
            ...
            <CardHeader>
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Badge className={ReturnService.getStatusColor(returnRequest.status)}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-4">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm">
                      <strong>Submitted:</strong> {formatDate(returnRequest.submittedAt)}
                    </span>
                  </div>
                  {returnRequest.processedAt && (
                    <div className="flex items-center gap-2">
                      <Calendar className="h-4 w-4 text-muted-foreground" />
                      <span className="text-sm">
                        <strong>Processed:</strong> {formatDate(returnRequest.processedAt)}
                      </span>
                    </div>
                  )}
                </div>
                <div className="space-y-4">
                  {returnRequest.refundAmount && (
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm">
                        <strong>Refund Amount:</strong> {ReturnService.formatCurrency(returnRequest.refundAmount)}
                      </span>
                    </div>
                  )}
                  {returnRequest.refundMethod && (
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm">
                        <strong>Refund Method:</strong> {returnRequest.refundMethod}
                      </span>
                    </div>
                  )}
                </div>
          ...
          {returnRequest.decisionNotes && (
            <Card className={returnRequest.status === ReturnStatus.APPROVED ? "border-green-200 bg-green-50" : returnRequest.status === ReturnStatus.DENIED ? "border-red-200 bg-red-50" : ""}>
              <CardHeader>
                <CardTitle className="flex items-center gap-2">
          ...
          {returnRequest.returnMedia && Array.isArray(returnRequest.returnMedia) && returnRequest.returnMedia.length > 0 && (
            <Card>
              <CardHeader>
                <CardTitle className="flex items-center gap-2">
              ...
              <CardContent>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {returnRequest.returnMedia.map((media: MediaAttachment, index: number) => (
              ...
              <div className="space-y-4">
                {returnRequest.returnItems && returnRequest.returnItems.length > 0 ? (
                    ...
                    <div className="flex-1">
                      ...
                      <div className="mt-2 space-y-1">
                        <p className="text-sm text-muted-foreground">
                          ...
                          {item.unitPrice && ` • Unit Price: ${ReturnService.formatCurrency(item.unitPrice)}`}
                        </p>
                        {item.totalPrice && (
                          <p className="text-sm font-medium">
                            Total: {ReturnService.formatCurrency(item.totalPrice)}
                          </p>
                        )}
                  ...
                  <div className="text-center py-8">
                    ...
                    <p className="text-muted-foreground">No items found for this return request.</p>
                  </div>
                )}
              </div>
            </CardContent>
              ...
              <div className="space-y-4">
                ...
                <div className="flex items-center gap-3">
                  <div className={`w-3 h-3 rounded-full ${
                    returnRequest.status !== ReturnStatus.PENDING ? "bg-green-500" : "bg-gray-300"
                  }`}></div>
                  <div>
                    <p className="text-sm font-medium">Under Review</p>
                    <p className="text-xs text-muted-foreground">
                      {returnRequest.status !== ReturnStatus.PENDING ? "Completed" : "In progress"}
                    </p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-3">
                  <div className={`w-3 h-3 rounded-full ${
                    returnRequest.processedAt ? "bg-green-500" : "bg-gray-300"
                  }`}></div>
                  <div>
                    <p className="text-sm font-medium">Decision Made</p>
                    <p className="text-xs text-muted-foreground">
                      {returnRequest.processedAt ? formatDate(returnRequest.processedAt) : "Pending"}
                    </p>
                  </div>
                </div>

                {(returnRequest.status === ReturnStatus.APPROVED || returnRequest.status === ReturnStatus.PROCESSING || returnRequest.status === ReturnStatus.COMPLETED) && (
                  <>
                    <div className="flex items-center gap-3">
                      <div className={`w-3 h-3 rounded-full ${
                        returnRequest.status === ReturnStatus.COMPLETED ? "bg-green-500" : 
                        returnRequest.status === ReturnStatus.PROCESSING ? "bg-yellow-500" : "bg-blue-500"
                      }`}></div>
                      <div>
                        <p className="text-sm font-medium">Processing Return</p>
                        <p className="text-xs text-muted-foreground">
                          {returnRequest.status === ReturnStatus.COMPLETED ? "Completed" : 
                           returnRequest.status === ReturnStatus.PROCESSING ? "In progress" : "Ready to process"}
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-3">
                      <div className={`w-3 h-3 rounded-full ${
                        returnRequest.status === ReturnStatus.COMPLETED ? "bg-green-500" : "bg-gray-300"
                      }`}></div>
                      <div>
                        <p className="text-sm font-medium">Refund Processed</p>
                        <p className="text-xs text-muted-foreground">
                          {returnRequest.status === ReturnStatus.COMPLETED ? "Completed" : "Pending"}
                        </p>
                      </div>
                    </div>
                  </>
              ...
              <CardHeader>
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Badge className={ReturnService.getStatusColor(returnRequest.returnAppeal.status)}>
                      {returnRequest.returnAppeal.status}
                    </Badge>
                  </div>
                </div>
                <CardDescription>
                  Submitted on {formatDate(returnRequest.returnAppeal.submittedAt)}
                </CardDescription>
              </CardHeader>
              <CardContent>
                <div className="space-y-4">
                  <div>
                    <span className="text-sm font-medium text-blue-700">Appeal Reason:</span>
                    <p className="mt-1 text-sm text-blue-800">{returnRequest.returnAppeal.reason}</p>
                  </div>
                  {returnRequest.returnAppeal.description && (
                    <div>
                      <span className="text-sm font-medium text-blue-700">Description:</span>
                      <p className="mt-1 text-sm text-blue-800">{returnRequest.returnAppeal.description}</p>
                    </div>
                  )}
                  {returnRequest.returnAppeal.decisionNotes && (
                    <div className="p-3 bg-white rounded border">
                      <span className="text-sm font-medium text-gray-700">Appeal Decision:</span>
                      <p className="mt-1 text-sm">{returnRequest.returnAppeal.decisionNotes}</p>
                      {returnRequest.returnAppeal.processedAt && (
                        <p className="text-xs text-muted-foreground mt-2">
                          Processed on {formatDate(returnRequest.returnAppeal.processedAt)}
                        </p>
                      )}
                    </div>
                  )}
                  {returnRequest.returnAppeal.mediaAttachments && Array.isArray(returnRequest.returnAppeal.mediaAttachments) && returnRequest.returnAppeal.mediaAttachments.length > 0 && (
                    <div>
                      <span className="text-sm font-medium text-blue-700">Attachments:</span>
                      <div className="mt-2 space-y-2">
                        {returnRequest.returnAppeal.mediaAttachments.map((attachment: MediaAttachment, index: number) => (
          ...
          <Card className="bg-gray-50 border-gray-200">
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                {returnRequest.status === ReturnStatus.DENIED && !returnRequest.returnAppeal && (
                  <Button asChild className="bg-blue-600 hover:bg-blue-700">
                    <Link href={`/returns/appeal?returnId=${returnRequest.id}`} className="flex items-center gap-2">
