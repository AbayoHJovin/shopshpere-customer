...
export function DiscountPageClient({ discountId }: DiscountPageClientProps) {
  ...
  const updateUrlWithFilters = useCallback(
    (newFilters: FilterState) => {
      ...
      const queryString = urlParams.toString()
        ? `?${urlParams.toString()}`
        : "";
      router.replace(`/discounts/${discountId}${queryString}`, {
        scroll: false,
      });
    },
    [searchParams, router, discountId]
  );

  useEffect(() => {
    // Get discount data
    const discountData = getDiscountById(discountId);
